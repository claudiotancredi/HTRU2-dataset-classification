...
def modifiedDualFormulation(DTR, LTR, C, K, piT):
...
class SVM ():
    ...
    def train (self, DTR, LTR, option, c=0, d=2, gamma=1.0, C=1.0, K=1.0, piT=0.9 ):
        self.option = option
        self.DTR = DTR
        self.LTR = LTR
        self.K = K
        self.C = C
        if (option == 'linear'):
            self.w = modifiedDualFormulation(DTR, LTR, self.C, self.K, piT=piT)
