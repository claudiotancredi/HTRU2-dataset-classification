...
class GMM():
    def train (self, D, L, components):
        ...
        GMM0_init = [(1.0, D0.mean(axis=1).reshape((D0.shape[0], 1)), utils.constrainSigma(np.cov(D0).reshape((D0.shape[0], D0.shape[0]))))]
        GMM1_init = [(1.0, D1.mean(axis=1).reshape((D1.shape[0], 1)), utils.constrainSigma(np.cov(D1).reshape((D1.shape[0], D1.shape[0]))))]
       

        self.GMM0 = LBGalgorithm.LBGalgorithm (GMM0_init, D0, components)
        self.GMM1 = LBGalgorithm.LBGalgorithm (GMM1_init, D1, components)
